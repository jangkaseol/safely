...
function transformPlaceData(location: LocationRow, details: LocationDetailsRow | null): Place {
  return {
    id: location.id,
    name: location.name,
    ...
    ai_analysis_content: details?.safety_analysis_basis || details?.ai_recommendations || null,
  }
}
...
export async function getPlaces(category?: string, searchQuery?: string) {
  try {
    ...
    const transformedData: Place[] = data.map((row: any) =>
      transformPlaceData(row as LocationRow, row.location_details as LocationDetailsRow | null),
    )
...
export async function getPlaceById(id: string) {
  try {
    ...
    const transformedData = transformPlaceData(data as LocationRow, data.location_details as LocationDetailsRow | null)
